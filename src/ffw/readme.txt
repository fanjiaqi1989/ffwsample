...
scene = view + data +logic;

scene功能：通过自身logic，控制所包含的view和data。

view被动显示和反馈输出--》scene
data保持数据，并受scene控制并反馈数据。

scene的逻辑层实现当前场景的全部逻辑控制。完成视图与逻辑、数据的三方分离。
...
2018.03.08
utils包中添加代码loadingmask类，用于显示加载通用遮罩
utils包中添加代码ResLoadingView类，用于模块化加载资源组
